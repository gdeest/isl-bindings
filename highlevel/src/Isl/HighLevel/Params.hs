...
--
-- Mirrors ISL's parameter alignment: when two sets with different
-- parameter spaces are combined, the result has the union of parameters.
--
-- Both input lists must be sorted; the result is sorted.
...
-- Used for 'alignParams' — you can only align to a superset parameter space.
