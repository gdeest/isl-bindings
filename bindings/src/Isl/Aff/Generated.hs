...
coefficientSgn aff typ pos =
    unsafePerformIO $ fromIntegral <$> c_coefficientSgn aff typ (fromIntegral pos)
...
dim aff typ =
    unsafePerformIO $ fromIntegral <$> c_dim aff typ
...
involvesDims aff typ first n =
    unsafePerformIO $ fromIntegral <$> c_involvesDims aff typ (fromIntegral first) (fromIntegral n)
...
involvesLocals aff =
    unsafePerformIO $ fromIntegral <$> c_involvesLocals aff
...
getDimName aff typ pos =
    unsafePerformIO $ C.peekCString =<< c_getDimName aff typ (fromIntegral pos)
...
isNan :: AffRef -> Bool
isNan aff =
    unsafePerformIO $ M.toBool <$> c_isNan aff
...
plainIsZero aff =
    unsafePerformIO $ M.toBool <$> c_plainIsZero aff
...
toStr :: AffRef -> String
toStr aff =
    unsafePerformIO $ C.peekCString =<< c_toStr aff
...
isCst :: AffRef -> Bool
isCst aff =
    unsafePerformIO $ M.toBool <$> c_isCst aff
...
plainIsEqual aff1 aff2 =
    unsafePerformIO $ M.toBool <$> c_plainIsEqual aff1 aff2
