...
dim multi typ =
    unsafePerformIO $ fromIntegral <$> c_dim multi typ
...
findDimById multi typ id =
    unsafePerformIO $ fromIntegral <$> c_findDimById multi typ id
...
involvesDims multi typ first n =
    unsafePerformIO $ fromIntegral <$> c_involvesDims multi typ (fromIntegral first) (fromIntegral n)
...
plainCmp multi1 multi2 =
    unsafePerformIO $ fromIntegral <$> c_plainCmp multi1 multi2
...
getTupleName multi typ =
    unsafePerformIO $ C.peekCString =<< c_getTupleName multi typ
...
hasTupleId multi typ =
    unsafePerformIO $ M.toBool <$> c_hasTupleId multi typ
...
rangeIsWrapping multi =
    unsafePerformIO $ M.toBool <$> c_rangeIsWrapping multi
...
toStr ma =
    unsafePerformIO $ C.peekCString =<< c_toStr ma
...
involvesLocals multi =
    unsafePerformIO $ fromIntegral <$> c_involvesLocals multi
...
involvesNan multi =
    unsafePerformIO $ fromIntegral <$> c_involvesNan multi
...
size multi =
    unsafePerformIO $ fromIntegral <$> c_size multi
...
hasRangeTupleId multi =
    unsafePerformIO $ M.toBool <$> c_hasRangeTupleId multi
...
plainIsEqual multi1 multi2 =
    unsafePerformIO $ M.toBool <$> c_plainIsEqual multi1 multi2
