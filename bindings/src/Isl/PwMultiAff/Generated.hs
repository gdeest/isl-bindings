...
dim pma typ =
    unsafePerformIO $ fromIntegral <$> c_dim pma typ
...
involvesDims pma typ first n =
    unsafePerformIO $ fromIntegral <$> c_involvesDims pma typ (fromIntegral first) (fromIntegral n)
...
involvesNan pma =
    unsafePerformIO $ fromIntegral <$> c_involvesNan pma
...
involvesParamId pma id =
    unsafePerformIO $ fromIntegral <$> c_involvesParamId pma id
...
getDimName pma typ pos =
    unsafePerformIO $ C.peekCString =<< c_getDimName pma typ (fromIntegral pos)
...
getTupleName pma typ =
    unsafePerformIO $ C.peekCString =<< c_getTupleName pma typ
...
hasTupleId pma typ =
    unsafePerformIO $ M.toBool <$> c_hasTupleId pma typ
...
hasTupleName pma typ =
    unsafePerformIO $ M.toBool <$> c_hasTupleName pma typ
...
isEqual pma1 pma2 =
    unsafePerformIO $ M.toBool <$> c_isEqual pma1 pma2
...
toStr pma =
    unsafePerformIO $ C.peekCString =<< c_toStr pma
...
involvesLocals pma =
    unsafePerformIO $ fromIntegral <$> c_involvesLocals pma
...
isaMultiAff pma =
    unsafePerformIO $ fromIntegral <$> c_isaMultiAff pma
...
nPiece pma =
    unsafePerformIO $ fromIntegral <$> c_nPiece pma
...
hasRangeTupleId pma =
    unsafePerformIO $ M.toBool <$> c_hasRangeTupleId pma
...
plainIsEqual pma1 pma2 =
    unsafePerformIO $ M.toBool <$> c_plainIsEqual pma1 pma2
