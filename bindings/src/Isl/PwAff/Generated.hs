...
dim pwaff typ =
    unsafePerformIO $ fromIntegral <$> c_dim pwaff typ
...
involvesDims pwaff typ first n =
    unsafePerformIO $ fromIntegral <$> c_involvesDims pwaff typ (fromIntegral first) (fromIntegral n)
...
involvesNan pa =
    unsafePerformIO $ fromIntegral <$> c_involvesNan pa
...
involvesParamId pa id =
    unsafePerformIO $ fromIntegral <$> c_involvesParamId pa id
...
nPiece :: PwAffRef -> Int
nPiece pwaff =
    unsafePerformIO $ fromIntegral <$> c_nPiece pwaff
...
plainCmp pa1 pa2 =
    unsafePerformIO $ fromIntegral <$> c_plainCmp pa1 pa2
...
getDimName pa typ pos =
    unsafePerformIO $ C.peekCString =<< c_getDimName pa typ (fromIntegral pos)
...
hasDimId pa typ pos =
    unsafePerformIO $ M.toBool <$> c_hasDimId pa typ (fromIntegral pos)
...
hasTupleId pa typ =
    unsafePerformIO $ M.toBool <$> c_hasTupleId pa typ
...
isCst :: PwAffRef -> Bool
isCst pwaff =
    unsafePerformIO $ M.toBool <$> c_isCst pwaff
...
isEmpty pwaff =
    unsafePerformIO $ M.toBool <$> c_isEmpty pwaff
...
isEqual pa1 pa2 =
    unsafePerformIO $ M.toBool <$> c_isEqual pa1 pa2
...
toStr pa =
    unsafePerformIO $ C.peekCString =<< c_toStr pa
...
isaAff :: PwAffRef -> Int
isaAff pa =
    unsafePerformIO $ fromIntegral <$> c_isaAff pa
...
plainIsEqual pwaff1 pwaff2 =
    unsafePerformIO $ M.toBool <$> c_plainIsEqual pwaff1 pwaff2
