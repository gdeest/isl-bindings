...
cmpLastNonZero c1 c2 =
    unsafePerformIO $ fromIntegral <$> c_cmpLastNonZero c1 c2
...
dim constraint typ =
    unsafePerformIO $ fromIntegral <$> c_dim constraint typ
...
involvesDims constraint typ first n =
    unsafePerformIO $ fromIntegral <$> c_involvesDims constraint typ (fromIntegral first) (fromIntegral n)
...
plainCmp c1 c2 =
    unsafePerformIO $ fromIntegral <$> c_plainCmp c1 c2
...
getDimName constraint typ pos =
    unsafePerformIO $ C.peekCString =<< c_getDimName constraint typ (fromIntegral pos)
...
isDivConstraint constraint =
    unsafePerformIO $ M.toBool <$> c_isDivConstraint constraint
...
isEqual constraint1 constraint2 =
    unsafePerformIO $ M.toBool <$> c_isEqual constraint1 constraint2
...
isEquality constraint =
    unsafePerformIO $ M.toBool <$> c_isEquality constraint
...
isLowerBound constraint typ pos =
    unsafePerformIO $ M.toBool <$> c_isLowerBound constraint typ (fromIntegral pos)
...
isUpperBound constraint typ pos =
    unsafePerformIO $ M.toBool <$> c_isUpperBound constraint typ (fromIntegral pos)
