...
newtype Conj2 = Conj2 (Conjunction Integer) deriving (Show)
newtype Conj3 = Conj3 (Conjunction Integer) deriving (Show)
newtype MapConj11 = MapConj11 (Conjunction MapDim) deriving (Show)
newtype MapConj12 = MapConj12 (Conjunction MapDim) deriving (Show)
...
genBoundedConjunction :: Int -> Gen (Conjunction Integer)
genBoundedConjunction nDims = do
  let bounds = concatMap (\i ->
        [ InequalityConstraint (Ix (fromIntegral i))
        , InequalityConstraint (Add (Constant 10) (Mul (-1) (Ix (fromIntegral i))))
        ]) [0 .. nDims - 1]
  nExtra <- choose (0, 3)
  extras <- vectorOf nExtra (genInequalityConstraint nDims)
  return $ Conjunction (bounds ++ extras)

-- | Generate a random inequality constraint over @nDims@ Integer-indexed dims.
genInequalityConstraint :: Int -> Gen (Constraint Integer)
...
genExpr :: Int -> Gen (Expr Integer)
genExpr nDims = do
  coeffs <- mapM (\i -> do
    c <- choose (-5, 5)
    return (c, fromIntegral i)) [0 .. nDims - 1]
...
genMapConjunction :: Int -> Int -> Gen (Conjunction MapDim)
...
genMapInequalityConstraint :: Int -> Int -> Gen (Constraint MapDim)
