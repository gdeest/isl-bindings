...
toPureQueryDecl (ISLFunction annots t name params) hsName = do
  ...
  let cFunName = "c_" ++ hsName
      ...
      -- All pure queries use a strict let to force the result inside
      -- unsafePerformIO.  This prevents use-after-free when callers
      -- lazily bind the result and then free the underlying ISL object.
      body
        ...
        | otherwise = case t of
          CHAR_PTR ->
            "    let !r = unsafePerformIO $ C.peekCString =<< " ++ cFunName ++ " " ++ unwords cCallArgs ++ " in r"
          _ ->
            "    let !r = unsafePerformIO $ " ++ retConv ++ cFunName ++ " " ++ unwords cCallArgs ++ " in r"
