...
-- ═══════════════════════════════════════════════════════════════════════
-- §6. V2 entry point — structural copy of 'Alpha.Core' Named fields
-- ═══════════════════════════════════════════════════════════════════════
--
...
--
-- The fan-out for top-level 'V2.Case' bodies (one statement per branch,
-- @eqName__brI@) and the synthetic body name for 'V2.Reduce'
-- (@eqName__body_K@) match 'lowerSystem' exactly so that downstream
-- consumers ('logicalName', 'Alpha.Polyhedral.Dependence',
-- 'Alpha.Codegen.lowerScheduleMaps') keep working unchanged.

-- | V2 entry point: lower an elaborated 'V2.System sys a' to the same
-- @(domains, writes, reads, projections)@ tuple as 'lowerSystem'.
--
-- The returned 'NamedSet's and 'NamedMap's are structurally compatible
-- with the surface lowering's output: tuple names are installed by
-- this walker (Elaborate leaves them anonymous), constraints come
-- straight from the elaborated 'Named' payloads, and the Case fan-out
-- and Reduce body-naming convention is preserved.
lowerSystemV2
  :: forall sys a.
     V2.System sys a
  -> ([NamedSet], [NamedMap], [NamedMap], [NamedMap])
...
-- | Read-extraction context for the V2 walker.  Mirrors 'ReadCtx' but
-- without the carried domain-constraints field — every emitted read
-- map already inherits the pctx-folded constraints from the
-- elaborator's 'Named m IslMap' payload.
...
extractBranchReadsV2 ctx c (V2.BCons _ _ body rest) =
  -- Nested (non-top-level) 'Case' is not fanned out: the surface
  -- 'extractReads' walks each branch in the current ambient and
  -- accumulates reads/projs.  V2 mirrors that — top-level 'Case'
  -- fan-out happens in 'lowerOneEqV2' via 'normalizeCases' upstream.
